...
Class Action {
	...
	function save_contractor(){
		...
		// echo $data;exit;
		if(empty($id)){
			$save = $this->db->query("INSERT INTO contractors set $data");
		}else{
			$save = $this->db->query("UPDATE contractors set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function update_jobs(){
		...
		$sql = "UPDATE jobs SET date = '$date', daily_rate = '$daily_rate' WHERE id = $job_id";
	...
	function delete_jobs(){
		...
		$delete = $this->db->query("DELETE FROM jobs where id = $job_id");
		if($delete){
			return 1;
		}
	}
	...
	function delete_contractor(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM contractors where id = $id");
		if($delete){
			return 1;
		}
	}
